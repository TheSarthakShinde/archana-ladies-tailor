...
// POST /api/appointments  (public - anyone can book)
router.post('/', async (req, res) => {
  try {
    const { name, phone, email, clothingType, preferredDate, measurements, instructions } = req.body;
    if (!name || !phone || !clothingType) {
      return res.status(400).json({ message: 'Name, phone, and clothing type are required' });
    }
    const appointment = await Appointment.create({ name, phone, email, clothingType, preferredDate, measurements, instructions });
